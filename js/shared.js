    ...
    return new Promise((resolve) => {
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            ...
            if (user) {
                ...
                // Redirect if not on the main login page to avoid a loop
...
export function renderHeaderAndNav(activePage) {
    ...
    const headerNavHTML = `
        <header class="flex justify-between items-center mb-8">
            <div class="flex items-center gap-2 sm:gap-4">
                <div class="bg-teal-600 p-2 sm:p-3 rounded-xl text-white shadow-lg">
                    <svg class="h-6 w-6 sm:h-8 sm:w-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <rect x="2" y="7" width="20" height="14" rx="2" ry="2"></rect>
                        <path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"></path>
                    </svg>
                </div>
                <div>
                    <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white">Errum Enterprise</h1>
                    <p id="user-email" class="text-xs sm:text-sm text-slate-500 dark:text-slate-400"></p>
                </div>
            </div>
            <div class="flex items-center gap-1 sm:gap-2">
                <button id="settings-btn" title="Settings" class="p-2 rounded-full text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-800">
                ...
                <button id="logout-btn" title="Logout" class="p-2 rounded-full text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-800">
                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    // Attach listeners for the newly added buttons
    document.getElementById('logout-btn')?.addEventListener('click', () => {
        signOut(auth).catch(error => console.error("Logout Error:", error));
    });

    document.getElementById('settings-btn')?.addEventListener('click', () => {
        // This is a placeholder. You would create a modal for this.
