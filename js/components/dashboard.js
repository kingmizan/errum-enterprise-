...
import { showTransactionDetails } from './transactionDetail.js';
...
function getDashboardTemplate() {
    return `
        ...
        <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-800">
            <div class="p-4 border-b dark:border-slate-800 flex justify-between items-center flex-wrap gap-4"><h2 class="text-xl font-bold">Recent Transactions</h2><input id="search-input" type="text" placeholder="Search party or item..." class="w-full sm:w-48 p-2 border rounded-lg bg-slate-50 dark:bg-slate-800 border-slate-300 dark:border-slate-700"></div>
...
function renderTransactionHistory() {
    ...
    tbody.innerHTML = pageItems.map(t => {
        ...
        if (t.type === 'trade') {
            ...
            actionsHtml = `
                <button title="Edit" data-action="edit" data-id="${t.id}" class="p-2 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-full">✏️</button>
                <button title="Delete" data-action="delete" data-id="${t.id}" class="p-2 text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-900/50 rounded-full">🗑️</button>
            `;
        } else { // Payment
            ...
            actionsHtml = `<button title="Delete" data-action="delete" data-id="${t.id}" class="p-2 text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-900/50 rounded-full">🗑️</button>`;
        }

        return `
            <tr data-id="${t.id}" class="cursor-pointer border-b dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
...
function initializeDashboardListeners() {
    ...
    document.getElementById('transaction-history-body')?.addEventListener('click', e => {
        ...
        if (button) {
            ...
            if (action === 'edit') { window.location.hash = `transaction-form?id=${id}`; }
            if (action === 'delete') {
