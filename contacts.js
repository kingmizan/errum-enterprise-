...
export function renderContacts() {
    ...
    state.contacts.forEach(c => {
        ...
        const typeBadge = c.type === 'buyer'
            ? `<span class="inline-flex items-center gap-1.5 py-1 px-2.5 rounded-full text-xs font-medium bg-teal-100 text-teal-800 dark:bg-teal-500/20 dark:text-teal-400">Buyer</span>`
            : `<span class="inline-flex items-center gap-1.5 py-1 px-2.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-300">Supplier</span>`;

        const row = document.createElement('tr');
        row.className = 'odd:bg-slate-50 dark:odd:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-800/50 border-b dark:border-slate-800 md:border-b-0';
        row.innerHTML = `
...
export function handleContactsClick(e) {
...
export function setupContactFormForEdit(id) {
...
export async function handleSaveContact(e) {
    ...
    if (!id && state.contacts.some(c => c.name.toLowerCase() === name.toLowerCase())) { showToast('A contact with this name already exists.'); return; }
    const contactData = {
        name,
