...
import { handleDelete, openPaymentModal, showTransactionDetailsModal, setupTradeFormForEdit } from './transactions.js';
...
export function renderTransactionHistory(data) {
    ...
    if (pageData.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center py-12 text-slate-500 dark:text-slate-400">No Transactions Found</td></tr>`;
        return;
    }
    ...
    pageData.forEach(t => {
        ...
        if (t.type === 'trade') {
            ...
            receivableBalHtml = `<span class="font-semibold ${receivableBalance > 0.01 ? 'text-green-600 dark:text-green-500' : 'text-slate-500'}">৳${receivableBalance.toFixed(2)}</span>`;
            ...
            payableBalHtml = receivableBalHtml = `<span class="text-slate-400">-</span>`;
            actionsHtml = `<div class="flex justify-end md:justify-center items-center gap-1"><button title="Delete" data-delete-id="${t.id}" class="p-1 text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-900/50 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button></div>`;
        }
        row.innerHTML = `<td data-label="Date" class="py-4 px-4 align-top">${t.date}</td><td data-label="Details" class="py-4 px-4 align-top">${detailsHtml}</td><td data-label="Profit/Value" class="py-4 px-4 align-top text-right font-medium">${valueHtml}</td><td data-label="Payable Bal" class="py-4 px-4 align-top text-right">${payableBalHtml}</td><td data-label="Receivable Bal" class="py-4 px-4 align-top text-right">${receivableBalHtml}</td><td data-label="Actions" class="py-4 px-4 align-top actions-cell">${actionsHtml}</td>`;
...
export function renderDashboardPaginationControls(totalItems) {
    ...
    controlsContainer.innerHTML = `<button id="prev-page-btn" class="px-3 py-1 text-sm rounded-md font-semibold bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 disabled:opacity-50" ${prevDisabled}>Previous</button><span class="text-sm font-semibold">Page ${state.dashboardCurrentPage} of ${totalPages}</span><button id="next-page-btn" class="px-3 py-1 text-sm rounded-md font-semibold bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 disabled:opacity-50" ${nextDisabled}>Next</button>`;
    document.getElementById('prev-page-btn')?.addEventListener('click', () => { if (state.dashboardCurrentPage > 1) { state.dashboardCurrentPage--; renderAll(); } });
    document.getElementById('next-page-btn')?.addEventListener('click', () => { if (state.dashboardCurrentPage < totalPages) { state.dashboardCurrentPage++; renderAll(); } });
};
...
export function handleDashboardClick(e) {
    ...
    if (button && button.closest('td')?.classList.contains('actions-cell')) {
        ...
        if (editId) { navigateTo('transaction-form').then(() => setupTradeFormForEdit(editId)); }
        if (deleteId) handleDelete(deleteId);
        if (paymentId) openPaymentModal(paymentId, paymentType);
    } else {
        const row = e.target.closest('tr[data-id]');
        if (row) showTransactionDetailsModal(row.dataset.id);
    }
}
